...
BMM150::BMM150(device::Device *interface, const I2CSPIDriverConfig &config) :
    I2CSPIDriver(config),
...
void BMM150::RunImpl()
{
    ...
    case STATE::READ: {
            ...
            if (_interface->read(cmd, (uint8_t *)&buffer, sizeof(buffer)) == PX4_OK) {
...
void BMM150::RegisterWrite(Register reg, uint8_t val)
{
    uint8_t buf = val;

    _interface->write((uint8_t)reg, &buf, 1);
}
...
uint8_t BMM150::RegisterRead(Register reg)
{
    ...
    int ret = _interface->read((uint8_t)reg, &buf, 1);
    if (ret != PX4_OK) {
        PX4_DEBUG("register read 0x%02hhX failed, ret = %d", (uint8_t)reg, ret);
        return -1;
    }
